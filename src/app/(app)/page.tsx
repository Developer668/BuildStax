...
export default async function DashboardPage() {
  ...
  const metrics = [
    { label: "Active pipeline", value: data.activeBusinesses.length.toString(), detail: `${data.dueToday.length} actions due`, icon: Activity, tone: "text-[#285df5] bg-[#eef2ff]" },
    { label: "Quoted value", value: formatCurrency(data.pipelineValueCents), detail: "Open + accepted quotes", icon: Banknote, tone: "text-[#247a4a] bg-[#edf8f1]" },
    { label: "Positive outcomes", value: `${data.conversionRate}%`, detail: "Of contacted businesses", icon: PhoneCall, tone: "text-[#9b5b0f] bg-[#fff6e8]" },
    { label: "Collected", value: formatCurrency(data.collectedCents), detail: "Recorded full payments", icon: CheckCircle2, tone: "text-[#7b467c] bg-[#f7edf7]" },
  ];
  ...
  return (
    <>
      ...
      {isSandbox() ? (
        <div className="mb-4 flex flex-col gap-3 rounded-[6px] border border-[#e5cda1] bg-[#fff8e9] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <section aria-label="Workspace metrics" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => (
          <div key={metric.label} className="panel flex min-h-28 items-start justify-between p-4">
            <div><div className="text-[10px] font-bold text-muted-foreground">{metric.label}</div><div className="mono mt-2 text-[25px] font-semibold leading-none">{metric.value}</div><div className="mt-2 text-[10px] text-muted-foreground">{metric.detail}</div></div>
            <div className={`grid size-8 place-items-center rounded-[5px] ${metric.tone}`}><metric.icon className="size-4" /></div>
          </div>
        ))}
      </section>
              ...
              {activePipelineStages.map((stage) => {
                ...
                return (
                  <div key={stage} className="flex min-h-40 flex-col justify-end">
                    ...
                    <div className="mx-auto w-full rounded-t-[3px] bg-[#dde3dc]" style={{ height: `${Math.max(8, (count / max) * 88)}px` }}><div className="h-1.5 w-full rounded-t-[3px] bg-[#91b83c]" /></div>
